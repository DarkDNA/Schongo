...
Searches through wikipedia for the given search string"""
        o = openUrl(
            "http://en.wikipedia.org/w/api.php?action=query&list=search&srsearch={}&srwhat=text&srlimit=3&format=xml".format(
        ...
        for i in xml.getElementsByTagName("p"):
            ...
            ctx.reply(
                "`B{}`B ( {} ) • {}".format(title, "http://wikipedia.org/wiki/{}".format(urllib.parse.quote(title)),
                                            snippet), "Wikipedia")
